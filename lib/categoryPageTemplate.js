...
document.getElementById('gen-form').addEventListener('submit', async (e) => {
  ...
  try {
    const res = await authFetch('/api/generate-npc', {
      method: 'POST',
    ...
    status.textContent = \`Generated: \${data.name} (\${data.roleArchetype} — \${data.faction})\${data.imageError ? ' — portrait failed: ' + data.imageError : ''}\`;
    setTimeout(() => window.location.reload(), data.imageError ? 6000 : 1200);
  } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => populateFactionSelect('gen-field-3', { includeUnaligned: true }));
</script>
`,
  enemies: `
<div class="sheet" style="margin: 0 0 40px; padding: 24px 32px;">
...
document.getElementById('gen-form').addEventListener('submit', async (e) => {
  ...
  status.style.display = 'block'; status.textContent = 'Calling the generator — this can take a bit for content + art…';
  try {
    const res = await authFetch('/api/generate-enemy', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: document.getElementById('gen-name').value,
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Generation failed');
    status.textContent = \`Generated: \${data.name} (\${data.tier} — \${data.faction})\${data.attributeBudgetWarning ? ' — note: ' + data.attributeBudgetWarning : ''}\${data.imageError ? ' — portrait failed: ' + data.imageError : ''}\`;
    setTimeout(() => window.location.reload(), data.imageError ? 6000 : 1200);
  } catch (err) {
    status.textContent = 'Error: ' + err.message;
  } finally {
...
document.addEventListener('DOMContentLoaded', () => populateFactionSelect('gen-field-3', { includeUnaligned: false }));
</script>
`,
  items: `
<div class="sheet" style="margin: 0 0 40px; padding: 24px 32px;">
...
document.getElementById('gen-form').addEventListener('submit', async (e) => {
  ...
  status.style.display = 'block'; status.textContent = 'Calling the generator — this can take a bit for content + art…';
  try {
    const res = await authFetch('/api/generate-item', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: document.getElementById('gen-name').value,
...
document.getElementById('gen-form').addEventListener('submit', async (e) => {
  ...
  status.style.display = 'block'; status.textContent = 'Calling the generator — this can take a bit for content + art…';
  try {
    const res = await authFetch('/api/generate-survivor', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: document.getElementById('gen-name').value,
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Generation failed');
    status.textContent = \`Generated: \${data.name} (The \${data.className})\${data.imageError ? ' — portrait failed: ' + data.imageError : ''}\`;
    setTimeout(() => window.location.reload(), data.imageError ? 6000 : 1200);
  } catch (err) {
    status.textContent = 'Error: ' + err.message;
  } finally {
...
document.getElementById('gen-form').addEventListener('submit', async (e) => {
  ...
  try {
    const res = await authFetch('/api/generate-log', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: document.getElementById('gen-name').value,
...
document.getElementById('gen-form').addEventListener('submit', async (e) => {
  ...
  try {
    const res = await authFetch('/api/generate-class', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name: document.getElementById('gen-name').value
