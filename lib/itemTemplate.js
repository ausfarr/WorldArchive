...
// calendarConfig is optional (Session Prep Companion, Phase 3) -- used
// only for QuestItem's createdDate/discoveredDate below.
function buildItemBodyHtml(item, imageUrl, calendarConfig) {
  const portraitBlock = `<img class="portrait-img" id="portrait-img-${item.id}" data-category="items" data-entry-id="${item.id}" data-label="Item render" src="${imageUrl || `images/${item.id}.png`}" alt="${escapeHtml(item.name)}" onerror="handlePortraitError(this)">`;
...
`;
    ...
    const locationLink = item.foundAtLocationId
      ? `<p><strong>Found at:</strong> <a href="dossier.html?category=locations&id=${escapeHtml(item.foundAtLocationId)}">View location</a></p>\n`
      : "";
    const { formatWorldDate } = require("./calendar");
    const dateLines = [];
    if (item.status) dateLines.push(`<strong>Status:</strong> ${escapeHtml(item.status)}`);
    if (item.createdDate) dateLines.push(`<strong>Created:</strong> ${escapeHtml(formatWorldDate(item.createdDate, calendarConfig))}`);
    if (item.discoveredDate) dateLines.push(`<strong>Discovered:</strong> ${escapeHtml(formatWorldDate(item.discoveredDate, calendarConfig))}`);
    const datesLine = dateLines.length ? `<p class="flavor">${dateLines.join(" &nbsp;|&nbsp; ")}</p>\n` : "";
    statsBlock = `<h2>Where It's Found / Why It Matters</h2>
${datesLine}<p>${escapeHtml(item.whereFoundWhyMatters)}</p>
${locationLink}`;
  }
...
function buildItemSubtitle(item) {
  if (item.category === "Weapon") {
    return `Weapon Skill: ${item.weaponSkillLabel || item.weaponSkill} — Weapon Type: ${item.weaponType}`;
  }
