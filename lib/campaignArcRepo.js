...
//
// Read-then-write, so it's wrapped in withLock() (see lib/asyncLock.js)
// keyed per arc: two Quests created off the same arc in quick succession
// (double-click, two open tabs) could otherwise both read the same
// questIds/pendingStages snapshot and each write back a single-item
// patch computed from it -- the second write wins outright, silently
// dropping the first Quest's link back to the arc (and/or leaving its
// pendingStages entry un-cleared). Locking serializes the two calls so
// the second one's read happens after the first's write has landed.
async function appendQuestToArc(worldId, arcId, questId, stageId) {
  return withLock(`campaign-arc:${worldId}:${arcId}`, async () => {
...
// Called from routes/campaignModule.js's DELETE handler when a Quest
// referenced by one or more Campaigns gets deleted. Same reasoning as
// campaignModuleRepo.js's removeEntryFromAllCampaignModules -- without
// this, quest_ids kept the dead id forever. Best-effort per arc (see
// call site).
//
// Same read-then-write shape as appendQuestToArc above, against the same
// questIds array -- locked per arc with the identical `campaign-arc:${worldId}:${arcId}`
// key so the two can't race each other either: deleting Quest A from Arc X
// (this function) at the same moment a new Quest is created off one of Arc
// X's unmatched stages (appendQuestToArc) could otherwise both read the
// same stale questIds snapshot, and whichever write lands second would
// silently undo the other's change.
async function removeQuestFromAllCampaignArcs(worldId, questId) {
  const arcs = await listCampaignArcs(worldId);
  for (const arc of arcs) {
    await withLock(`campaign-arc:${worldId}:${arc.id}`, async () => {
      const current = await getCampaignArc(worldId, arc.id);
      if (!current || !current.questIds.includes(questId)) return;
      await updateCampaignArc(worldId, arc.id, { questIds: current.questIds.filter((id) => id !== questId) });
    });
  }
}
...
module.exports = {
  ...
  appendQuestToArc,
  removeQuestFromAllCampaignArcs
};
