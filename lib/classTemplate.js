...
function buildClassBodyHtml(cls, imageUrl) {
  const portraitBlock = `<img class="portrait-img" id="portrait-img-${cls.id}" data-category="classes" data-entry-id="${cls.id}" data-label="Character portrait" src="${imageUrl || `images/${cls.id}.png`}" alt="${escapeHtml(cls.baseName)}" onerror="handlePortraitError(this)">`;
...
<tr><th>Location</th><td>${cls.evolutionEvent.locationId ? `<a href="dossier.html?category=locations&id=${escapeHtml(cls.evolutionEvent.locationId)}">${escapeHtml(cls.evolutionEvent.location)}</a>` : escapeHtml(cls.evolutionEvent.location)}</td></tr>
