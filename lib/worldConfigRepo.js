...
async function resetWorldConfig(worldId) {
  ...
  const { error } = await supabase
    .from("world_config")
    .update({ draft_json: {}, factions_json: [], lore_doc_ref: null, stat_system_json: null, skill_system_json: null, style_guide_json: null, category_config_json: null, setup_completed_at: null })
    .eq("world_id", worldId);
...
async function checkAndIncrementGenerationCount(worldId, cap = GENERATION_CAP) {
  await getOrCreateWorldConfig(worldId);
...
module.exports = { getOrCreateWorldConfig, getDraft, saveDraftStep, getFactions, saveFactions, resetWorldConfig, getStatSystem, saveStatSystem, getSkillSystem, saveSkillSystem, getStyleGuide, saveStyleGuide, getCategoryConfig, saveCategoryConfig, getFullConfig, markSetupComplete, getGenerationCount, checkAndIncrementGenerationCount, GENERATION_CAP, FREE_ENTRY_CAP, getEntriesPurchased, addPurchasedEntries, getLocationsMapLocked, setLocationsMapLocked };
