...
// can't clobber others saved moments earlier from the same step -- but
// that guard is only as good as the read it merges against, and
// routes/wizard.js's autosave endpoint fires on every field blur/change,
// not debounced/serialized client-side. Tabbing through several fields
// on one step fires several of these concurrently: without a lock, two
// in-flight calls can both read the same pre-merge draft_json, each
// merge their own field into their own JS copy, and whichever write
// lands second silently overwrites the first's field with a stale
// snapshot that never saw it. Same check-then-act shape already fixed
// for patchEntryMeta/entryLinker's rebake (entriesRepo.js) and the
// Campaign Arc/Quest cleanup helpers (campaignArcRepo.js/
// campaignModuleRepo.js) -- this call site was the one progressive-
// commit path that still had it. Locked per-world (not per-step) since
// every step's fields live in the same draft_json column on the same
// row -- a save to step 2 racing a save to step 1 would clobber each
// other's read the same way two saves to the same step would.
async function saveDraftStep(worldId, step, fields) {
  return withLock(`wizard-draft:${worldId}`, async () => {
    const config = await getOrCreateWorldConfig(worldId);
...
async function resetWorldConfig(worldId) {
  ...
  const { error } = await supabase
    .from("world_config")
    // calendar_config joined this list in bug batch 1, Phase 3: the
    // calendar is now a wizard step, so Start Over / auto-reset / Delete
    // World have to clear it like every other step's data (before, a
    // world restarted from scratch kept its old calendar).
    .update({ draft_json: {}, factions_json: [], lore_doc_ref: null, stat_system_json: null, skill_system_json: null, race_system_json: null, style_guide_json: null, category_config_json: null, generic_system_json: null, calendar_config: null, setup_completed_at: null, ruleset: "echoes" })
    .eq("world_id", worldId);
...
// Progressive-commit storage for the minimal Calendar (Session Prep
// Companion, Phase 2 -- see session_prep_companion_scope.md Section
// 4a-i). Same shape/placement as getStyleGuide/saveStyleGuide above.
async function getCalendarConfig(worldId) {
  const config = await getOrCreateWorldConfig(worldId);
  return config.calendar_config || null;
}

async function saveCalendarConfig(worldId, calendarConfig) {
  await getOrCreateWorldConfig(worldId);
  const { data, error } = await supabase
    .from("world_config")
    ...
    .select("*")
    .single();
  if (error) throw new Error(`saveCalendarConfig failed: ${error.message}`);
  return data.calendar_config;
}
