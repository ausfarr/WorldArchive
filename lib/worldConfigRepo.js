...
async function resetWorldConfig(worldId) {
  ...
  const { error } = await supabase
    .from("world_config")
    .update({ draft_json: {}, factions_json: [], lore_doc_ref: null, stat_system_json: null, skill_system_json: null, style_guide_json: null, category_config_json: null, setup_completed_at: null, ruleset: "echoes" })
    .eq("world_id", worldId);
...
// Not wrapped in a DB-level atomic increment like generation count --
// this is only ever called once per Stripe webhook event (effectively
// serial), so a plain read-modify-write is an acceptable simplification
// here, same tradeoff already made elsewhere in this codebase (e.g.
// saveFactions) for lower-concurrency writes.
async function addPurchasedEntries(worldId, amount) {
  const config = await getOrCreateWorldConfig(worldId);
  const newTotal = (config.entries_purchased || 0) + amount;
  const { data, error } = await supabase
    .from("world_config")
    .update({ entries_purchased: newTotal })
    .eq("world_id", worldId)
    .select("*")
    .single();
  if (error) throw new Error(`addPurchasedEntries failed: ${error.message}`);
  return data.entries_purchased;
}
...
module.exports = { getOrCreateWorldConfig, getDraft, saveDraftStep, getFactions, saveFactions, resetWorldConfig, getStatSystem, saveStatSystem, getSkillSystem, saveSkillSystem, getStyleGuide, saveStyleGuide, getCategoryConfig, saveCategoryConfig, getFullConfig, getRuleset, setRuleset, markSetupComplete, getGenerationCount, checkAndIncrementGenerationCount, refundGenerationCount, GENERATION_CAP, POINTS_PER_GENERATION, POINTS_PER_FIELD_ASSIST, FREE_ENTRY_CAP, getEntriesPurchased, addPurchasedEntries, getLocationsMapLocked, setLocationsMapLocked };
