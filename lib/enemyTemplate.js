...
function buildEnemyBodyHtml(enemy, imageUrl, statLabels) {
  ...
  // pattern used everywhere else stat labels are consumed.
  const labels = statLabels || {};
  const L = (key, fallback) => (labels[key] && labels[key].label) || fallback;
  const bodyL = L("body", "BODY");
  const reflexL = L("reflex", "REFLEX");
  const knowledgeL = L("knowledge", "KNOWLEDGE");
  const presenceL = L("presence", "PRESENCE");
  const sanityL = L("sanity", "SANITY");
  const fateL = L("fate", "FATE");

  const portraitBlock = `<img class="portrait-img" id="portrait-img-${enemy.id}" data-category="enemies" data-entry-id="${enemy.id}" data-label="Character portrait" src="${imageUrl || `images/${enemy.id}.png`}" alt="${name}" onerror="handlePortraitError(this)">`;
...
`
  ...
  // Session Prep Companion, Phase 7 -- Boss-tier only status field.
  const statusLine = (enemy.tier === "Boss" && enemy.status) ? `<p class="flavor"><strong>Status:</strong> ${escapeHtml(enemy.status)}</p>` : "";

  return `
${portraitBlock}
${quoteBlock}${statusLine}<p class="flavor">${escapeHtml(enemy.flavor)}</p>
<h2>Stat Block</h2>
<table class="stat-table">
<tr><th>${escapeHtml(bodyL)}</th><th>${escapeHtml(reflexL)}</th><th>${escapeHtml(knowledgeL)}</th><th>${escapeHtml(presenceL)}</th><th>${escapeHtml(sanityL)}</th><th>${escapeHtml(fateL)}</th></tr>
...
<tr><td>Max Health</td><td>${derived.maxHealth}</td><td class="formula">((${bodyL}×2)+(${sanityL}×2))+BASE</td></tr>
<tr><td>Max Energy</td><td>${derived.maxEnergy}</td><td class="formula">((${knowledgeL}×2)+(${fateL}×2))+BASE</td></tr>
<tr><td>Dodge Chance</td><td>${derived.dodgeChance}%</td><td class="formula">BASE+(${reflexL}/4)+(SKILL/4)</td></tr>
<tr><td>Crit Chance</td><td>${derived.critChance}%</td><td class="formula">BASE+(${fateL}/3)+(SKILL/5)</td></tr>
<tr><td>Accuracy</td><td>${derived.accuracy}%</td><td class="formula">BASE+(${reflexL}/2)+(SKILL/2)</td></tr>
<tr><td>Move Speed</td><td>${derived.moveSpeed}</td><td class="formula">BASE+(${reflexL}/5)+(SKILL/10)</td></tr>
</table>
<p class="flavor">BASE = ${derived.base} (provisional). SKILL is a flat stand-in for this enemy's effective skill investment, approximated at ${derived.effectiveSkill} per its ${enemy.tier}-tier baseline — not tied to any single named skill.</p>
<h2>Abilities</h2>
${abilitiesHtml}
${phaseChangeBlock}<h2>Combat Notes</h2>
...
function buildEnemyManifestEntry(enemy, factionLabel) {
  return {
    id: enemy.id,
    name: enemy.name,
    subtitle: `${enemy.tier} — ${factionLabel || "Unaligned"}`,
    tags: [],
    faction: enemy.faction,
    tier: enemy.tier,
    locked: false
