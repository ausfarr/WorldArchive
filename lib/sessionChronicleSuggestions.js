...
const { findOrCreatePendingUpdate } = require("./pendingEntryUpdatesRepo");

// Locations added alongside the original four (scope doc's Phase 7 only
// named npcs/factions/survivors/items) -- it's one of the 5 core
// Quest-slot categories (see sessionAssembly.js's ENTRY_READERS comment)
// and routinely takes session-implied state changes of its own (an
// outpost destroyed, a hideout discovered/abandoned), same as a faction
// losing territory. The regenerate/status_flip machinery downstream
// (routes/pendingUpdates.js, archive/js/render.js's REGENERATE_ENDPOINTS)
// was already fully generic per category -- this whitelist was the only
// thing excluding it.
const VALID_CATEGORIES = new Set(["npcs", "factions", "survivors", "items", "locations"]);
...
async function createSuggestionsFromChronicle(worldId, log) {
  ...
  for (const update of log.impliedUpdates) {
    ...
    // not just source. findOrCreatePendingUpdate does the existence check,
    // the refresh-if-the-facts-changed, *and* the insert under one lock
    // (see its own comment) -- two confirms of the same Chronicle landing
    // close together used to be able to both pass a standalone
    // find-then-create as unguarded steps and both insert.
    const row = await findOrCreatePendingUpdate(worldId, {
