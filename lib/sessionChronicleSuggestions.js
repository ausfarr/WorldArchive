...
async function createSuggestionsFromChronicle(worldId, log) {
  ...
  for (const update of log.impliedUpdates) {
    ...
    // findOrCreatePendingUpdate does the existence check *and* the insert
    // under one lock (see its own comment) -- two confirms of the same
    // Chronicle landing close together used to be able to both pass a
    // standalone find-then-create as unguarded steps and both insert.
