...
async function buildClassRosterContext(worldId) {
  ...
  // Missing until now, unlike every sibling buildXRosterContext above --
  // a world's first-ever class generation rendered this as "" instead of
  // an explanatory line, leaving a dangling "EXISTING ROSTER (...)"
  // header with nothing under it in prompts/classContentPrompt.js.
  if (lines.length === 0 && !overflowNote) {
    return "No classes archived yet — any concept/archetype is available.";
  }
  return lines.join("\n") + overflowNote;
}
...
// opts defaults to including locked ghost-placeholder factions (unlike
// every other readXManifest above), since several callers -- filling an
// existing faction (lib/factionDeepLore.js#generateFactionDeepLore),
// duplicate-avoidance/id-dedup (lib/factionDeepLore.js#createNewFaction,
// lib/proceduralGenerators.js#uniqueId) -- need ghosts visible on the
// manifest. Callers that offer this list as generation options a reader
// could click through to (lib/worldFlavor.js#getFactionOptions,
// routes/map.js's backdrop/anchor grounding) must pass
// `{ locked: false }` explicitly -- see the header comment on
// getFactionOptions() for why (same ghost-leak bug already fixed for PDF
// export in lib/pdfExport.js, just never applied here since this was the
// one readXManifest that never took an opts param at all).
async function readFactionManifest(worldId, opts) {
  return listEntries(worldId, "factions", opts);
}
...
// ---------- Spells ----------
//
// Added later than every category above (multi-ruleset genericization,
// Phase 4) -- routes/generateSpell.js originally built its roster context
// inline with a raw, uncapped `listEntries(worldId, "spells")` map/join
// instead of going through this file, so it never got the cap every
// sibling category has had since MAX_FULL_ROSTER_LINES was introduced.
// Harmless for a new world, but the exact same unbounded-growth cost
// problem documented at the top of this file once a world's spell list
// passes the cap -- fixed by routing it through splitRosterForCap like
// everything else. `school` isn't mirrored onto the top-level manifest
// row the way `level` is (see spellRepo.js's entryMeta) -- only `level`
// is used here, matching what the inline version it replaces showed.

async function readSpellManifest(worldId, opts) {
  return listEntries(worldId, "spells", opts);
}

async function buildSpellRosterContext(worldId) {
  const manifest = await readSpellManifest(worldId, { locked: false });
  const { recent, overflow } = splitRosterForCap(manifest);
  const lines = recent.map((m) => `- ${m.id} | ${m.name}: Level ${m.level != null ? m.level : "?"}`);
  const overflowNote = plainOverflowNote(overflow);
  if (lines.length === 0 && !overflowNote) {
    return "No spells archived yet -- any concept is available.";
  }
  return lines.join("\n") + overflowNote;
}
