...
const { getSkillSystem } = require("./worldConfigRepo");
...
// mimeType defaults to PNG for the same reason as saveDungeonMapImage
// above -- the generate path always produces one, the new upload path
// (routes/map.js's /map/upload-backdrop) passes the real uploaded type.
...
async function saveSurvivorEntry(worldId, survivor, imageUrl) {
  ...
  const statLabels = await getStatLabels(worldId);
  const bodyHtml = buildSurvivorBodyHtml(survivor, imageUrl, factionLabel, statLabels);
