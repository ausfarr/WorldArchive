...
// ---------- Map biome tiles (Option C map fix, several per world) ----------
...
// establishing shot vs. several illustrated biome tiles meant to be
// composited together with soft-blended edges (see archive/js/mapLayout.js
// and archive/map.html). A world only gets tiles for whichever
// biomeTags its locations actually use (see prompts/locationContentPrompt.js's
// BIOME_TAGS) -- never all of them speculatively.
function slugifyBiomeTag(biomeTag) {
  return String(biomeTag || "unknown")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "") || "unknown";
}

async function saveMapTile(worldId, biomeTag, imageBuffer) {
  if (!imageBuffer) return null;
  const objectPath = `${worldId}/tile-${slugifyBiomeTag(biomeTag)}.png`;
  const { error } = await supabase.storage
    .from(MAP_BACKDROP_BUCKET)
    .upload(objectPath, imageBuffer, { contentType: "image/png", upsert: true });
  if (error) throw new Error(`saveMapTile(${biomeTag}) failed: ${error.message}`);
  const { data } = supabase.storage.from(MAP_BACKDROP_BUCKET).getPublicUrl(objectPath);
  return data.publicUrl;
}

async function mapTileExists(worldId, biomeTag) {
  const { data: files, error } = await supabase.storage.from(MAP_BACKDROP_BUCKET).list(worldId);
  if (error) throw new Error(`mapTileExists(${biomeTag}) failed: ${error.message}`);
  const target = `tile-${slugifyBiomeTag(biomeTag)}.png`;
  return !!(files || []).find((f) => f.name === target);
}

function getMapTileUrl(worldId, biomeTag) {
  const objectPath = `${worldId}/tile-${slugifyBiomeTag(biomeTag)}.png`;
  const { data } = supabase.storage.from(MAP_BACKDROP_BUCKET).getPublicUrl(objectPath);
  return data.publicUrl;
}
