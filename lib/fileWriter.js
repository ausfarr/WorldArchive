...
async function saveFactionEntry(worldId, faction, roundupRows) {
  ...
  // Same carry-forward reasoning as accentColor just above, for a second
  // patchEntryMeta-only field: bannerImageUrl (routes/worldArt.js's
  // AI-generated faction banner) also lives outside `faction` entirely,
  // so without this it was silently deleted by the very next Deep Lore
  // regenerate-confirm or entryLinker.js backfill rebake -- discarding a
  // real Gemini image generation, not just a cosmetic reset like the
  // accent-color default.
  const bannerImageUrl = (existing && existing.bannerImageUrl) || null;
...
async function saveLocationEntry(worldId, location, imageUrl) {
  ...
  // dungeonMap (routes/dungeonMap.js's baked battle map, real Gemini
  // image spend) and manualMapPosition (routes/entries.js's dragged
  // world-map pin) both live outside `location` entirely -- they're
  // written straight onto raw_json by patchEntryMeta(), never passed
  // through here. upsertEntry() below is a full row overwrite, so
  // without carrying them forward, every regenerate-confirm AND every
  // entryLinker.js backfill rebake of this Location silently deleted
  // both -- same shape of bug already fixed for factions' accentColor
  // (see saveFactionEntry() above), just not yet ported to locations.
  const existing = await getEntry(worldId, "locations", location.id);
  const dungeonMap = (existing && existing.dungeonMap) || null;
  const manualMapPosition = (existing && existing.manualMapPosition) || null;
