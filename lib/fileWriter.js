...
async function saveNpcEntry(worldId, npc) {
  ...
  const manifestFields = buildNpcManifestEntry(npc);
  const entryMeta = {
    category: "npcs",
    id: npc.id,
    name: npc.name,
    ...
    raw: npc,
    footer: [`Faction: ${npc.faction || "Unaligned"}`, "Source: generated via World Forge pipeline"],
    bodyHtml
  };
...
async function saveEnemyEntry(worldId, enemy) {
  ...
  const entryMeta = {
    ...
    raw: enemy,
    footer: [`Faction: ${enemy.faction || "Unaligned"}`, "Source: generated via World Forge pipeline"],
    bodyHtml
  };
...
async function saveLogEntry(worldId, log) {
  ...
  const manifestFields = buildLogManifestEntry(log);
  const entryMeta = {
    category: "logs",
    id: log.id,
    name: log.name,
