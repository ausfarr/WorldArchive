...
// ---------- Map tiles (Option C map fix, several per world) ----------
// Sits alongside the single shared backdrop above in the same bucket --
// both are map art, just different granularity: one photographic
// establishing shot vs. several illustrated tiles meant to be composited
// together with soft-blended edges (see archive/js/mapLayout.js and
// archive/map.html). One tile per faction actually represented among a
// world's locations (plus "unaligned" if any location has none), each
// grounded in that faction's own territory description -- see
// routes/map.js's buildTileSubject(). (This function set originally
// keyed tiles by a biome tag instead of a faction; renamed for clarity
// after that pivot, but the storage mechanics are unchanged -- it was
// always just a generic string key.)
function slugifyTileKey(key) {
...
async function saveMapTile(worldId, tileKey, imageBuffer) {
  if (!imageBuffer) return null;
  const objectPath = `${worldId}/tile-${slugifyTileKey(tileKey)}.png`;
  const { error } = await supabase.storage
    .from(MAP_BACKDROP_BUCKET)
    .upload(objectPath, imageBuffer, { contentType: "image/png", upsert: true });
  if (error) throw new Error(`saveMapTile(${tileKey}) failed: ${error.message}`);
  const { data } = supabase.storage.from(MAP_BACKDROP_BUCKET).getPublicUrl(objectPath);
  return data.publicUrl;
}

async function mapTileExists(worldId, tileKey) {
  const { data: files, error } = await supabase.storage.from(MAP_BACKDROP_BUCKET).list(worldId);
  if (error) throw new Error(`mapTileExists(${tileKey}) failed: ${error.message}`);
  const target = `tile-${slugifyTileKey(tileKey)}.png`;
  return !!(files || []).find((f) => f.name === target);
}

function getMapTileUrl(worldId, tileKey) {
  const objectPath = `${worldId}/tile-${slugifyTileKey(tileKey)}.png`;
  const { data } = supabase.storage.from(MAP_BACKDROP_BUCKET).getPublicUrl(objectPath);
  return data.publicUrl;
}

// Used by the "Delete World" flow -- removes every tile.png alongside
// the shared backdrop, without needing to know which biomeTags this
// world happened to generate (same list-then-filter technique as
// deleteAllPortraits).
