...
// referenceImage (optional) is { buffer, mimeType } -- when present, its
// inlineData part goes in BEFORE the text part, which is how this Gemini
// image model does image-conditioned editing rather than from-scratch
// generation: given an existing image plus text instructions, it edits
// that image instead of inventing a new one. See generateImage() below
// for why routes/generateEntryImage.js's keepLikeness option feeds an
// entry's own current portrait back in here.
async function attemptGenerateImage(prompt, apiKey, imageConfig, referenceImage) {
  const parts = [];
  if (referenceImage) {
    parts.push({ inlineData: { mimeType: referenceImage.mimeType, data: referenceImage.buffer.toString("base64") } });
  }
...
async function generateImage(prompt, { imageSize, aspectRatio: aspectRatioOverride, referenceImage } = {}) {
  ...
  try {
    const result = await attemptGenerateImage(prompt, apiKey, imageConfig, referenceImage);
    logImageCost();
    return result;
  } catch (firstErr) {
