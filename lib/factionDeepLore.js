...
const { getFactions, getStyleGuide, getCalendarConfig } = require("./worldConfigRepo");
...
const { formatCalendarContextForPrompt, proposeAndValidateDate, resolveRegeneratedDate } = require("./calendar");

const HEX_COLOR_RE = /^#[0-9a-fA-F]{6}$/;

// Every top-level key prompts/factionContentPrompt.js's SCHEMA_DESCRIPTION
// asks for -- Deep Lore is one of the largest single-call JSON payloads in
// the app, and these are schema-ordered toward the end, so they're the
// fields most likely to get cut if the model runs out of maxTokens
// mid-object (see lib/claude.js's callClaudeExpectingJson requiredKeys
// param, and session_addendum_beta_feedback_batch3.md Fix 4). Passed to
// both Deep Lore call sites below (generateFactionDeepLore and
// createNewFaction) so a completeness failure retries automatically
// instead of silently saving a faction missing its Relationships/Economy/
// Joining sections.
const DEEP_LORE_REQUIRED_KEYS = [
  "nickname", "overviewQuote", "origin", "corePhilosophy", "structureHierarchy",
  "territory", "goalsNearTerm", "goalsLongTerm", "internalTensions", "iconography",
  "relationships", "economyResources", "joining"
];
...
async function generateFactionDeepLore(worldId, fillExistingId, revisionNote) {
  ...
  const calendarContext = formatCalendarContextForPrompt(calendarConfig);
  ...
  const deepLore = await callClaudeExpectingJson({
    systemPrompt: contentSystemPrompt,
    userMessage: "Generate the Deep Lore section now.",
    maxTokens: 4000,
    requiredKeys: DEEP_LORE_REQUIRED_KEYS
  });

  // Session Prep Companion, Phase 3 -- model proposes, code validates
  // before this ever reaches storage. A regenerate that omits/loses a
  // previously-set foundingDate falls back to whatever was already
  // there, same "regenerate shouldn't silently wipe an established
  // fact" treatment as saveFactionEntry's own accentColor carry-forward.
  const foundingDate = resolveRegeneratedDate(deepLore.foundingDate, calendarConfig, priorRaw && priorRaw.foundingDate);
...
async function createNewFaction(worldId, { name, concept } = {}) {
  ...
  const seed = await callClaudeExpectingJson({
  ...
  const faction = {
    ...
    ...deepLore,
    foundingDate: proposeAndValidateDate(deepLore.foundingDate, calendarConfig),
    accentColor: null
  };
  ...
  try {
    ...
    const parsed = await callClaudeExpectingJson({
...
//
// Deliberately NOT called from inside the wizard's parallel first-pass
// upgrade (routes/wizardReview.js) -- that runs every faction's Deep Lore
...
// an update. The wizard instead calls it afterwards, one faction at a
// time, from lib/afterEntrySave.js's linkWizardFactionsSequentially().
// Safe to call from any single, sequential faction save (new-faction
// creation, manual regenerate, that post-pass).
async function syncReciprocalRelationships(worldId, faction) {
  ...
  for (const rel of relationships) {
    try {
      ...
      // Audit item 5: match by resolved id FIRST, then by name. Name-only
      // matching meant a renamed faction's next save appended a second
      // reciprocal (under the new name) to every faction that already had
      // one pointing at the same toId -- two graph edges, the faction
      // listed twice in Deep Lore.
      const alreadyHasIt = (targetRaw.relationships || []).some((r) => (r.toId && r.toId === faction.id) || r.faction === faction.name);
      if (alreadyHasIt) continue;

      // target.faction || target.id, NOT bare target.id -- same fallback
      // generateFactionDeepLore() above uses. A faction's archive `.id`
      // (dossier-URL slug) and its `.faction` matching key (what every
      // OTHER entry's `faction` field must equal to be counted in this
      // faction's Roundup) can legitimately differ -- see
      // worldFlavor.js#getFactionOptions's comment on Austin's migrated
      // Echoes world (`ferro_kings` vs `the-ferro-kings`). Writing bare
      // target.id here silently overwrote a real matching key with the
      // slug on every reciprocal-relationship sync, orphaning every
      // entry already tagged with the real key from that faction's
      // Roundup -- with no error, since the write itself succeeds fine.
