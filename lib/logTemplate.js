...
function buildLogBodyHtml(log) {
  ...
  const hexTongueBlock = log.hexTongue
    ? `<h2>Intercepted Signal — Hex-Tongue (Glitch-Kin network traffic)</h2>
<table class="rel-table">
<tr><th>Translation Tier</th><th>Text</th></tr>
<tr><td>Unreadable</td><td>${escapeHtml(log.hexTongue.unreadable)}</td></tr>
<tr><td>Keyword</td><td>${escapeHtml(log.hexTongue.keyword)}</td></tr>
<tr><td>Phrase</td><td>"${escapeHtml(log.hexTongue.phrase)}"</td></tr>
</table>
<p class="flavor"><strong>Human interpretation (in-fiction):</strong> ${escapeHtml(log.hexTongue.humanInterpretation)}</p>
<p class="flavor"><strong>Linguist interpretation:</strong> ${escapeHtml(log.hexTongue.linguistInterpretation)}</p>
`
    : "";
...
${preBlock}
${hexTongueBlock}<h2>Design Notes</h2>
...
function buildLogManifestEntry(log) {
  const factionLabel = log.faction ? FACTION_LABEL[log.faction] : "Personal";
  return {
    id: log.id,
    name: log.name,
    subtitle: `${LOG_TYPE_LABEL[log.logType]} — ${factionLabel}`,
    tags: [],
    faction: log.faction || null,
    locked: false
  };
