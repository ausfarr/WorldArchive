...
// PF2e NPCs (added alongside 5e's) tag their combatProfile with
// `ruleset: 'pf2e'` (see lib/rulesets/pf2e/npcCombatDefaults.js and
// homebrewEnemyGenerator.js) so this can pick the right embedded
// renderer -- every 5e combatProfile predates that field (both the
// original default template and every Combatant-upgraded profile
...
function combatProfileBlock(npc) {
  ...
  const { buildEmbeddedCombatProfileHtml } = npc.combatProfile.ruleset === "pf2e"
    ? require("./rulesets/pf2e/enemyTemplate")
    : require("./rulesets/5e/enemyTemplate");
