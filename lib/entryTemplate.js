...
function buildBodyHtml(npc) {
  const name = escapeHtml(npc.name);
  const factionId = FACTION_CATEGORY_ID[npc.faction];

  const portraitBlock = `<img class="portrait-img" src="images/${npc.id}.png" alt="${name}" onerror="this.outerHTML='&lt;div class=&quot;portrait-slot&quot;&gt;Character portrait — pending&lt;span class=&quot;sub&quot;&gt;art prompt not yet generated&lt;/span&gt;&lt;/div&gt;'">`;
...
function buildManifestEntry(npc) {
  return {
    id: npc.id,
    name: npc.name,
    subtitle: `${npc.roleArchetype} — ${FACTION_LABEL[npc.faction] || "Unaligned"}`,
    tags: [],
    faction: npc.faction,
    roleArchetype: npc.roleArchetype,
    locked: false
