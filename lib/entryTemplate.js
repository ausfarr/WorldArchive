...
function lifeDatesBlock(npc, calendarConfig) {
  ...
  const lines = [];
  // Session Prep Companion, Phase 7 -- status field, same row as the
  // Phase 3 life dates it's naturally grouped with.
  if (npc.status) lines.push(`<strong>Status:</strong> ${escapeHtml(npc.status)}`);
