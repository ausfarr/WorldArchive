...
async function backfillReferencesFromNewEntry(worldId, newCategory, newEntry) {
  ...
  for (const category of ALL_CATEGORIES) {
    ...
    for (const row of rows) {
      ...
      // patchEntryMeta-only fields (dungeonMap, manualMapPosition on
      // Locations; accentColor, bannerImageUrl on Factions) no longer get
      // dropped if a rebake lands AFTER that patch -- saveLocationEntry()/
      // saveFactionEntry() (lib/fileWriter.js) now read the existing row
      // and carry those fields forward before this rebake's full raw_json
      // overwrite runs. See patchEntryMeta's own NOTE (entriesRepo.js) for
      // the fuller history. Sharing the lock still separately guarantees
      // this rebake's write is never based on content staler than the
