...
function formatWorldDate(date, calendarConfig) {
  ...
  // Bug batch 1, Phase 4: lore-extracted dates carry precision
  // ('year'|'month'|'day') and approximate; the unused parts of a
  // year/month-precision date are placeholders, never shown. Dates
  // without these fields (every date before Phase 4) render as before.
...
// DM-facing label for each date field, used by lib/timelineEvents.js's
// auto-generated Timeline event summaries ("Born: Vess Okoro", "Founded:
// The Ashen Hand") -- kept next to DATE_FIELDS_BY_CATEGORY since it's
// directly derived from the same field list and a new date field should
// never need a label update in a different file.
...
// Deep-equal for two WorldDate-shaped values (or null/undefined) --
// used to detect whether a date field genuinely changed on this save
// (lib/timelineEvents.js's createEntryDateEvents) rather than comparing
// object references, since a save's incoming entry is always a fresh
// object even when its date fields are unchanged.
function worldDatesEqual(a, b) {
  if (a === b) return true;
  if (!a || !b) return false;
...
// Structural validation for a whole calendar_config before it's saved --
// moved here from routes/wizardCalendar.js (bug batch 1, Phase 3) so
// lib/calendarPresets.js's test can hold every preset to exactly the same
// bar a DM-edited calendar has to clear on save. Returns an error string,
// or null if the shape is valid.
function validateCalendarConfigShape(calendarConfig) {
  ...
  if (calendarConfig.weekday_names != null) {
    ...
    if (calendarConfig.weekday_names.some((w) => typeof w !== "string" || !w.trim())) {
      return "Every weekday needs a name.";
    }
  }
...
// Placeholder weekday name used when a generated list comes back short.
// Deliberately obvious ("Day 6") -- archive/js/calendarEditor.js
// highlights any weekday input still matching WEEKDAY_PLACEHOLDER_RE so
// the DM knows exactly which ones to rename.
const WEEKDAY_PLACEHOLDER_RE = /^Day \d+$/;

// Repairs a model-proposed weekday list to exactly daysPerWeek names:
// extras are truncated, missing ones padded with "Day N" placeholders.
// Bug batch 1, Phase 3: routes/wizardCalendar.js used to set
// weekday_names to null on ANY count mismatch, which the Calendar page
// then rendered as "D1..D7" column headers -- the "wrong week names"
// report. A near-miss list (the model returning 6 names for a 7-day
// week) is far more useful kept and patched than thrown away.
function repairWeekdayNames(names, daysPerWeek) {
  const cleaned = (Array.isArray(names) ? names : [])
    .map((w) => (w == null ? "" : String(w).trim()))
    .filter(Boolean)
    .slice(0, daysPerWeek);
  while (cleaned.length < daysPerWeek) cleaned.push(`Day ${cleaned.length + 1}`);
  return cleaned;
}

// Bug batch 1, Phase 3: before a calendar CHANGE is saved, the editor
// warns if the new months/days (or a moved current year, via
// validateWorldDate's before/after bounds) would make already-stored
// dates invalid. Pure: callers pass in what's stored, this only counts --
// nothing is ever mutated or migrated (Austin's call: warn, don't touch
// existing data).
//   timelineEvents: [{ worldDate }]            (lib/timelineRepo.js shape)
//   notableDates:   [{ monthIndex, day }]      (calendarNotableDatesRepo.js shape)
//   entries:        [{ category, raw }]        (entriesRepo.listEntries shape)
// Returns { timelineEvents, notableDates, entryDateFields, total, examples }.
function countDatesInvalidatedByCalendar(calendarConfig, { timelineEvents = [], notableDates = [], entries = [] }) {
  const examples = [];
  const note = (text) => { if (examples.length < 5) examples.push(text); };

  let timelineCount = 0;
  for (const e of timelineEvents) {
    if (!e || !e.worldDate) continue;
    if (!validateWorldDate(e.worldDate, calendarConfig).valid) {
      timelineCount++;
      note(`Timeline: ${e.summary || "(event)"}`);
    }
  }

  let notableCount = 0;
  for (const nd of notableDates) {
    if (!validateMonthDay(nd.monthIndex, nd.day, calendarConfig).valid) {
      notableCount++;
      note(`Notable date: ${nd.name || "(unnamed)"}`);
    }
  }

  let entryFieldCount = 0;
  for (const entry of entries) {
    const fields = DATE_FIELDS_BY_CATEGORY[entry.category] || [];
    const raw = entry.raw || {};
    for (const field of fields) {
      if (raw[field] == null) continue;
      if (!validateWorldDate(raw[field], calendarConfig).valid) {
        entryFieldCount++;
        note(`${entry.name || entry.id}: ${DATE_FIELD_LABELS[field] || field}`);
      }
    }
  }
  ...
  return {
    ...
    notableDates: notableCount,
    entryDateFields: entryFieldCount,
    total: timelineCount + notableCount + entryFieldCount,
    examples
  };
}

module.exports = { validateCalendarConfigShape, repairWeekdayNames, WEEKDAY_PLACEHOLDER_RE, countDatesInvalidatedByCalendar, formatWorldDate, validateWorldDate, validateMonthDay, proposeAndValidateDate, resolveRegeneratedDate, formatCalendarContextForPrompt, sanitizeEntryDateFields, DATE_FIELDS_BY_CATEGORY, DATE_FIELD_LABELS, worldDatesEqual, totalDaysInYear, ordinal, MAX_YEARS_BEFORE_CURRENT, MAX_YEARS_AFTER_CURRENT };
