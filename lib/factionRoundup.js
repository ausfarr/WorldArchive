...
async function buildFactionRoundup(worldId, factionKey) {
  ...
  const npcs = (await readNpcManifest(worldId)).filter((m) => !m.locked && m.faction === factionKey);
  for (const m of npcs) {
    rows.push({ type: "NPC", category: "npcs", id: m.id, name: m.name, note: m.roleArchetype || "" });
  }

  const enemies = (await readEnemyManifest(worldId))
  ...
  const logs = (await readLogManifest(worldId)).filter((m) => !m.locked && m.faction === factionKey);
  for (const m of logs) {
    rows.push({ type: "Log", category: "logs", id: m.id, name: m.name, note: "" });
  }

  const survivors = (await readSurvivorManifest(worldId)).filter((m) => !m.locked && m.faction === factionKey);
  ...
  const locations = (await readLocationManifest(worldId)).filter((m) => !m.locked && m.faction === factionKey);
