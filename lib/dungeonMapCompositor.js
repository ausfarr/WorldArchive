...
const PAGE_CONTENT_TIMEOUT_MS = 45000; // matches lib/pdfExport.js's explicit value; see its comment
...
async function compositeGridOntoImage(imageBuffer, mimeType, gridSize) {
  // Queued behind the same shared Chromium concurrency limit
  // lib/pdfExport.js uses (see lib/asyncLock.js) -- both files launch a
  // full headless Chromium process per call and draw from the same
  // finite CPU/memory budget.
