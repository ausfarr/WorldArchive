...
async function buildExportHtml(worldId, scope, params, includeImages) {
  ...
  if (scope === "category") {
    ...
    // so no per-entry getEntry() round trip is needed here. { locked: false }
    // excludes ghost placeholders (lib/entryLinker.js's ensureGhostPlaceholder()
    // auto-creates these for any name reference to an ungenerated entry --
    // real name, null bodyHtml/subtitle) the same way every other reader of
    // this table already does (lib/roster.js's context builders, the category
    // grid page's client-side filter, lib/factionRoundup.js) -- omitting it
    // rendered a near-blank sheet for each one, dumping content the user
    // never generated into a PDF meant to only cover what they actually have.
    const entries = await listEntries(worldId, params.category, { locked: false });
    const label = await resolveCategoryLabel(worldId, params.category);
    const cover = `<section class="pdf-cover"><p class="sheet-eyebrow">${escapeHtml(
      worldTitle
  ...
  for (const cat of CATEGORY_ORDER) {
    // { locked: false } -- same ghost-placeholder exclusion as the
    // category-scope branch above.
    const entries = await listEntries(worldId, cat, { locked: false });
