...
async function buildExportHtml(worldId, scope, params, includeImages) {
  ...
  if (scope === "campaign") {
    ...
    const cover = `<section class="pdf-cover">
      <p class="sheet-eyebrow">${escapeHtml(worldTitle)} — Campaign Module</p>
    ...
    for (const ref of mod.entries || []) {
      ...
      const block = buildEntryBlock(entry, includeImages);
    ...
    return {
      ...
      filename: `${slug(mod.name)}-campaign-module.pdf`
    };
  }
