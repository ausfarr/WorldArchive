...
const REGISTRY = {
  ...
  pf2e: {
    ...
    classes: {
      formulas: require("./pf2e/classFormulas"),
      template: require("./pf2e/classTemplate")
      // Homebrew tier ONLY -- see prompts/rulesets/pf2e/classContentPrompt.js's
      // header for why the model designs its own classDcSchedule/goodSaves
      // (a real per-class PF2e design choice) while every other
      // proficiency track uses this project's own fixed default curve.
    },
    items: {
      formulas: require("./pf2e/itemFormulas"),
      template: require("./pf2e/itemTemplate")
      // Homebrew tier ONLY -- see prompts/rulesets/pf2e/itemContentPrompt.js's
      // header. Price guidance is explicitly labeled an ESTIMATE (see
      // itemFormulas.js's header for why no official price-by-level
      // table is asserted as fact here), the same "estimated" honesty
      // this project applies to 5e's Challenge Rating.
    },
    spells: {
      formulas: require("./pf2e/spellFormulas"),
      template: require("./pf2e/spellTemplate")
      // Homebrew tier ONLY -- see prompts/rulesets/pf2e/spellContentPrompt.js's
      // header. Rank-by-level and Heightened (+N) scaling are both
      // verified real formulas; no prompt slot here for the same
      // multi-tier-prompt reason 5e enemies' doesn't use one -- routes/
      // generateSpell.js requires the prompt module directly.
    },
    survivors: {
      formulas: require("./pf2e/survivorFormulas"),
      template: require("./pf2e/survivorTemplate")
      // Homebrew tier ONLY -- a PC is a real pf2e Class instance
      // (classId referencing a Phase 9 Classes entry), HP/Class DC/
      // Perception/saves computed from that class's actual data via
      // survivorFormulas.js's computePcProfile(). Category slug stays
      // "survivors", same scoping decision 5e's Phase 8 made.
    }
  },
  generic: {
    enemies: {
