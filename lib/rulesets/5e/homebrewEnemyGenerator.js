...
// exist for the same reason rosterOverride already did (see its own
// use a few lines below): a caller with no real worldId to read this
// world's own setting/factions/lore from -- currently just the
// unauthenticated demo
// generator (routes/demo.js), which grounds a Homebrew 5e monster in a
// lightweight genre preset instead of a real world's wizard data. Real
// callers (routes/generateEnemy.js, lib/campaignEntryGenerators.js,
// routes/npcCombatant.js's Combatant upgrade) never pass these, so their
// behavior is byte-for-byte unchanged -- "reuse it, don't fork it," per
// this file's own header comment, rather than a parallel demo-only
// generator function.
async function generateHomebrew5eEnemy(worldId, { name, faction, targetCr, campaignContext, rosterOverride, settingContextOverride, factionOptionsTextOverride, loreContextOverride } = {}) {
  const settingContext = settingContextOverride != null ? settingContextOverride : await getSettingContext(worldId);
  const factionOptionsText = factionOptionsTextOverride != null ? factionOptionsTextOverride : formatFactionOptionsForPrompt(await getFactionOptions(worldId));
  const loreContext = loreContextOverride != null ? loreContextOverride : await getLoreContext(worldId, { category: "enemies", faction });
