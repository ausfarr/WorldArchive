...
function buildItemBodyHtml(item, imageUrl) {
  ...
  const sourceBadge = item.sourceMode
    ? `<p class="flavor" style="text-transform:uppercase; font-size:0.7rem; letter-spacing:0.05em;">Source: ${escapeHtml(item.sourceMode)}${item.srdSourceId ? ` — SRD "${escapeHtml(item.srdSourceId)}"` : ""}</p>`
    : "";
  const licenseNote = item.srdLicenseNote
    ? `<p class="flavor" style="font-size:0.7rem; color:var(--ink-faint);">${escapeHtml(item.srdLicenseNote)}</p>`
    : "";
...
function buildItemManifestEntry(item) {
  return {
    ...
    tags: [`<span class="tag">${escapeHtml(item.rarity || "Mundane")}</span>`, `<span class="tag">${escapeHtml(item.itemType)}</span>`, `<span class="tag">${escapeHtml(item.sourceMode || "homebrew")}</span>`],
