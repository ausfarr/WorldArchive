...
function buildClassBodyHtml(cls, imageUrl) {
  ...
  const sourceBadge = cls.sourceMode
    ? `<p class="flavor" style="text-transform:uppercase; font-size:0.7rem; letter-spacing:0.05em;">Source: ${escapeHtml(cls.sourceMode)}${cls.srdSourceId ? ` — SRD "${escapeHtml(cls.srdSourceId)}"` : ""}</p>`
    : "";
  const licenseNote = cls.srdLicenseNote
    ? `<p class="flavor" style="font-size:0.7rem; color:var(--ink-faint);">${escapeHtml(cls.srdLicenseNote)}</p>`
    : "";
...
function buildClassManifestEntry(cls) {
  return {
    ...
    tags: [`<span class="tag">5e Class</span>`, `<span class="tag">${escapeHtml(cls.sourceMode || "homebrew")}</span>`],
