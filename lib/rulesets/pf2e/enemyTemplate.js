...
// Compact version of the stat block for embedding inside an NPC entry's
// page (no portrait/eyebrow, since the NPC page already has its own) --
// same purpose as 5e's buildEmbeddedCombatProfileHtml(), used by
// lib/entryTemplate.js's combatProfileBlock() for pf2e-ruleset worlds'
// NPC "Combatant" default/upgrade profile (see
...
function buildEmbeddedCombatProfileHtml(enemy) {
  ...
  return `
<table class="rel-table">
<tr><th>Level</th><td>${enemy.level}${enemy.role ? ` — ${escapeHtml(enemy.role)}` : ""}</td></tr>
<tr><th>Perception</th><td>${formatModifier(enemy.perception)}</td></tr>
${skillsText ? `<tr><th>Skills</th><td>${skillsText}</td></tr>` : ""}
<tr><th>${abilityModifiersLine(enemy.abilities)}</th><td></td></tr>
<tr><th>AC</th><td>${enemy.armorClass}; <strong>Fort</strong> ${formatModifier(savingThrows.fort)}, <strong>Ref</strong> ${formatModifier(savingThrows.ref)}, <strong>Will</strong> ${formatModifier(savingThrows.will)}</td></tr>
<tr><th>HP</th><td>${enemy.hitPoints}</td></tr>
</table>
${strikeBlock(enemy.melee, "Melee")}
${strikeBlock(enemy.ranged, "Ranged")}
`;
...
module.exports = { buildEnemyBodyHtml, buildEmbeddedCombatProfileHtml, buildEnemyManifestEntry, slugify, escapeHtml, formatModifier };
