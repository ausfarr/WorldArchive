...
async function maybeCreateDateSuggestion(worldId, log, calendarConfig) {
  ...
  if (existing) {
    // A regenerate can also revise the resolved date itself, not just the
    // prose around it (e.g. corrected recap notes push an event to a
    // different day) -- if the still-PENDING row's proposed date text has
    // actually changed, refresh it in place instead of leaving the first-
    // draft date suggestion stale forever. An applied/dismissed row is
    // left untouched either way (see sessionChronicleSuggestions.js's
    // identical guard for the same reasoning).
    if (existing.status === "pending" && existing.deltaText !== deltaText) {
      return updatePendingUpdate(worldId, existing.id, { deltaText, payload: existing.payload });
    }
    return null;
  }
