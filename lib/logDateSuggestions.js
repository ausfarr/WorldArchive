...
async function maybeCreateDateSuggestion(worldId, log, calendarConfig) {
  ...
  // findOrCreatePendingUpdate does the existence check, the insert, *and*
  // the refresh-in-place when a regenerate revised the resolved date
  // itself (not just the prose around it) under one lock (see its own
  // comment) -- two confirms of the same Log landing close together used
  // to be able to both pass a standalone find-then-create as unguarded
  // steps and both insert.
  ...
  return findOrCreatePendingUpdate(worldId, {
