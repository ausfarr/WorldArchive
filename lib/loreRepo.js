...
async function backfillFactionTags(worldId, factionIds) {
  ...
  const { error: coreError } = await supabase
    ...
    .eq("world_id", worldId)
    .eq("core", true);
  if (coreError) throw new Error(`backfillFactionTags (core) failed: ${coreError.message}`);

  const { error: factionCategoryError } = await supabase
    .from("lore_sections")
    .update({ faction_tags: factionIds })
    .eq("world_id", worldId)
    .contains("category_tags", ["factions"]);
  if (factionCategoryError) throw new Error(`backfillFactionTags (factions category) failed: ${factionCategoryError.message}`);
}
