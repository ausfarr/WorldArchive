...
async function backfillFactionTags(worldId, factionIds) {
  ...
  // One batched update via .in() instead of one round trip per matched
  // section -- every matched row gets the exact same faction_tags value,
  // so there's no need for this part to be per-row like the filtering
  // above (which stays in JS for the real PostgREST .contains() reason
  // documented above -- this doesn't touch that).
  const { error } = await supabase
    .from("lore_sections")
    .update({ faction_tags: factionIds })
    .in("id", toUpdate.map((s) => s.id));
  if (error) throw new Error(`backfillFactionTags update failed: ${error.message}`);
}

// Deletes all of a world's lore_sections rows -- used by the wizard's
// session-based auto-reset and explicit "Start Over" action alongside
// worldConfigRepo.resetWorldConfig (which handles draft_json/
// factions_json/lore_doc_ref on the other table).
async function clearLoreSections(worldId) {
  const { error } = await supabase
    .from("lore_sections")
    .delete()
    .eq("world_id", worldId);
  if (error) throw new Error(`clearLoreSections failed: ${error.message}`);
}

module.exports = { listLoreSections, replaceLoreSections, backfillFactionTags, clearLoreSections };
