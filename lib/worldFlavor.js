...
const { getDraft, getFactions, getStatSystem, getStyleGuide, getSkillSystem } = require("./worldConfigRepo");
...
async function getSettingContext(worldId) {
  ...
  const parts = [
    ...
    // Free-text pitch from Step 1 (added so a vague idea like "wild west
    // but the gold is alive" reaches every content generation, not just
    // the wizard's own Generate buttons).
    s1.worldDescription ? `World description (the creator's own pitch): ${s1.worldDescription}` : null,
...
//
// { locked: false } excludes ghost-placeholder factions -- a locked stub
// lib/entryLinker.js#ensureGhostPlaceholder() auto-creates the moment any
// OTHER piece of content name-references a faction that doesn't exist
// yet. Without this filter, every content-generation call in the app
// (NPCs/enemies/items/locations/classes/survivors/logs/spells/procedural
// faction-picking) could offer that ghost as a selectable faction id --
// directly contradicting this function's own "a reader could actually
// click through to" contract above, since a ghost's dossier is an empty
// stub (null bodyHtml). Same ghost-leak bug already fixed for PDF export
// (lib/pdfExport.js) and every other roster context builder in
// lib/roster.js -- this was the one place still missing it, since
// readFactionManifest() never took a `locked` option before now.
//
...
// Formats the fixed field-skill pool (generated once in Wizard Step 5,
// see prompts/wizardSkillSystemPrompt.js) for injection into class/item
// generation prompts, so the model picks FROM this list rather than
// inventing new skills on every call -- the fix for a large roster
// slowly accumulating near-duplicate skill names.
function formatFieldSkillsForPrompt(skillSystem) {
  const skills = (skillSystem && skillSystem.fieldSkills) || [];
  if (!skills.length) {
    return "(no fixed skill pool generated yet for this world -- invent 3-5 flavor skills fitting this world's professions and setting)";
  }
...
// Formats the world's own weapon-skill display names alongside their
// fixed canonical keys -- prompts still select/output the canonical key
// (item.weaponSkill needs it for the damage-range clamp lookup in
// lib/itemFormulas.js), this just gives the model this world's own name
// for each so flavor text/design notes can reference it naturally.
function formatWeaponSkillsForPrompt(skillSystem) {
  const labels = (skillSystem && skillSystem.weaponSkills) || {};
  return WEAPON_SKILL_CANONICAL_KEYS
    .map((key) => `- ${key} (this world calls it: "${labels[key] || key}")`)
    .join("\n");
}

// Resolves a single canonical weapon-skill key to this world's own
// display label -- used at render/save time (see lib/fileWriter.js's
// item tags) so the LIVE archive shows "Greatblades," not "Heavy
// Weapons," while item.weaponSkill itself stays the fixed canonical
// string internally. Falls back to the canonical English name if this
// world hasn't generated a skill system yet.
function resolveWeaponSkillLabel(skillSystem, canonicalKey) {
...
module.exports = {
  ...
  getStyleGuide,
  getSkillSystem,
  formatFieldSkillsForPrompt,
  formatWeaponSkillsForPrompt,
  resolveWeaponSkillLabel,
  WEAPON_SKILL_CANONICAL_KEYS,
  DEFAULT_STAT_LABELS
};
