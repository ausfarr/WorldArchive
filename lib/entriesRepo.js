...
// saveFactionEntry() and saveLocationEntry() (lib/fileWriter.js) now
// explicitly carry forward every patchEntryMeta-only field that exists
// today (accentColor, bannerImageUrl, dungeonMap, manualMapPosition) by
// reading the existing row first -- so THIS specific gap is closed for
// every field currently in use. The architectural gap remains open for
// any *future* patchEntryMeta-only field on any category: whoever adds
// one needs to add the same carry-forward to that category's own
// save*Entry() writer, same as these two.
