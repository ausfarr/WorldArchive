...
// "locations" was added to this list after it was originally written --
// see session_addendum_location_lore_grounding_shipped.md for the
// category-list-drift bug this caused (same class of bug already fixed
// for routes/export.js's VALID_CATEGORIES, lib/pdfExport.js's
// CATEGORY_ORDER, and lib/roster.js's spell roster cap): Location
// generation (routes/generateLocation.js calls getLoreContext with
// category: "locations") could only ever ground on core:true sections,
// since no code path ever tagged a non-core section with "locations" --
// History/Founding, Faction/Politics, Culture, Resources, and
// Technology/Magic lore all silently never reached a Location prompt.
const ALL_CATEGORIES = ["factions", "npcs", "enemies", "classes", "items", "logs", "survivors", "locations"];
...
const TOPIC_CATEGORY_MAP = [
  ...
  { re: /resource|econom|trade|currency|scarcity/i, categories: ["factions", "items", "locations"], core: false },
  { re: /cultur|religion|custom|ritual|language/i, categories: ["npcs", "survivors", "factions", "locations"], core: false },
  { re: /technolog|magic|supernatural|power system/i, categories: ["items", "classes", "enemies", "locations"], core: false },
  { re: /histor|timeline|collapse|founding|origin/i, categories: ["factions", "npcs", "locations"], core: false },
  { re: /faction|politic|government|military/i, categories: ["factions", "npcs", "enemies", "locations"], core: false },
