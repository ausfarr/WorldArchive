...
// Stripe statuses that mean the subscription is over for good -- Stripe
// is no longer trying to collect, so there's no quota to wait for.
// past_due is deliberately NOT here: Stripe is still retrying the card,
// and the existing behavior (subscription path, quota paused, credits
// still spendable) is left alone by decision -- see the Phase 2 section
// of the addendum for why that's flagged rather than changed.
const LAPSED_SUBSCRIPTION_STATUSES = new Set(["canceled", "unpaid", "incomplete_expired"]);

// 'free'       -- no subscriptions row at all
// 'lapsed'     -- a row whose status is in LAPSED_SUBSCRIPTION_STATUSES
// 'subscribed' -- any other row (active, past_due, trialing, ...), same
//                 path every row took before this fix
...
// (middleware/enforceEntryCap.js, lib/regenerateGate.js) -- a lapsed row
// and a past_due row both fall back to the capped path, which is why
// this is a separate question from billingTierFor().
