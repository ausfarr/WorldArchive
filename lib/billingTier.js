...
// Audit item 2 safety net: a row still marked 'active' whose period
// ended more than this long ago is treated as lapsed. Normally every
// renewal (invoice.payment_succeeded) or cancellation
// (customer.subscription.deleted) moves the row on well before this --
// the only way to get here is a webhook that never arrived, which used to
// mean full paid access forever. Five days covers Stripe's own ~3-day
// webhook retry window with margin, so a merely-delayed renewal never
// locks a paying subscriber out.
const STALE_ACTIVE_GRACE_MS = 5 * 24 * 60 * 60 * 1000;

function isStaleActive(subscription, now = new Date()) {
  if (!subscription || subscription.status !== "active" || !subscription.current_period_end) return false;
  const end = new Date(subscription.current_period_end).getTime();
  return Number.isFinite(end) && end < now.getTime() - STALE_ACTIVE_GRACE_MS;
}

// 'free'       -- no subscriptions row at all
// 'lapsed'     -- a row whose status is in LAPSED_SUBSCRIPTION_STATUSES,
//                 or a stale 'active' row (isStaleActive)
// 'subscribed' -- any other row (active, trialing, ...), same path every
//                 row took before this fix
function billingTierFor(subscription, now = new Date()) {
  if (!subscription) return "free";
  if (LAPSED_SUBSCRIPTION_STATUSES.has(subscription.status)) return "lapsed";
  if (isStaleActive(subscription, now)) return "lapsed";
  return "subscribed";
}

// Stripe statuses where the Stripe subscription itself is finished --
// the only states in which starting a NEW subscription checkout can't
// double-bill (audit item 1). Everything else (active, past_due, unpaid,
// trialing, incomplete, paused) is a subscription Stripe may still charge.
...
function isActiveSubscription(subscription, now = new Date()) {
  return !!(subscription && subscription.status === "active" && !isStaleActive(subscription, now));
}
...
// Audit item 10: credits are stored in points (5 per generation) and
// shown in whole generations, so 1-4 leftover points used to read as
// "0 purchased credits" even though they still pay for field assists.
// creditExtraFieldAssists is that remainder, for the UI to show.
function creditFields(creditBalancePoints) {
  const points = Math.max(0, creditBalancePoints || 0);
  return {
    creditBalance: pointsToGenerations(points),
    creditExtraFieldAssists: points % POINTS_PER_GENERATION
  };
}
...
function buildFreeAllowanceFields(config, creditBalancePoints) {
  ...
  return {
    ...
    ...creditFields(creditBalancePoints),
