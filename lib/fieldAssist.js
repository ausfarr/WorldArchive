...
async function getFieldAssistSuggestion({ worldId, category, fieldId, currentEntryData }) {
  ...
  const quoteNote = isQuoteField(fieldId) ? `\n\n${QUOTE_CRAFT_GUIDANCE}` : "";

  const systemPrompt = `You are helping a GM fill in ONE field on an in-progress entry for their tabletop/game world archive. Write ONLY the suggested content for that field -- no preamble, no quotation marks around it, no explanation, no markdown. Just the raw text that belongs in the field.
...
RELEVANT LORE:
${loreContext || "(no specific lore sections tagged for this category/faction yet)"}${quoteNote}`;
