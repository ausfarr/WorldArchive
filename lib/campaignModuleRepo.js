...
//
// Read-then-write against `entries`, so it's locked per module with the
// same `campaign-module:${worldId}:${moduleId}` key routes/campaignModule.js's
// PATCH handler now also locks on -- without this, two entries referenced
// by the same Quest deleted in quick succession (or a delete racing a
// PATCH that replaces `entries` wholesale from a client's own snapshot)
// could each compute their patch off a stale array, and the second write
// would silently undo the first's removal. Same shape of fix as
// campaignArcRepo.js's appendQuestToArc/removeQuestFromAllCampaignArcs.
async function removeEntryFromAllCampaignModules(worldId, category, entryId) {
  ...
  for (const mod of modules) {
    await withLock(`campaign-module:${worldId}:${mod.id}`, async () => {
      const current = await getCampaignModule(worldId, mod.id);
      if (!current) return;
      const filtered = current.entries.filter((e) => !(e.category === category && e.entryId === entryId));
