...
const { DEFAULT_NPC_COMBAT_PROFILE: DEFAULT_NPC_COMBAT_PROFILE_PF2E } = require("./rulesets/pf2e/npcCombatDefaults");
const {
  getSettingContext,
  getFactionOptions,
...
async function createNewNpc(worldId, { name, role, faction, campaignContext } = {}) {
  ...
  // Multi-ruleset genericization, Phase 7 (+ PF2e expansion): every NPC
  // in a 5e- or pf2e-ruleset world gets a lightweight default combat
  // profile so it's never a hard dead-end if players attack it -- see
  ...
  } else if (npcRuleset === "pf2e") {
    npc.combatProfile = DEFAULT_NPC_COMBAT_PROFILE_PF2E;
  }
