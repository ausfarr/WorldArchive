...
const { buildRosterContext, buildLocationRosterContext, buildItemRosterContext, buildLogRosterContext } = require("./roster");
...
const { saveNpcEntry, saveLocationEntry, saveItemEntry, saveLogEntry } = require("./fileWriter");
...
async function createNewNpc(worldId, { name, role, faction, campaignContext } = {}) {
  ...
  const contentRaw = await callClaude({ systemPrompt: contentSystemPrompt, userMessage: "Generate the NPC now.", maxTokens: 3000 });
  let npc;
  try {
...
async function createNewLocation(worldId, { name, regionBiome, faction, campaignContext } = {}) {
  ...
  const contentRaw = await callClaude({ systemPrompt: contentSystemPrompt, userMessage: "Generate the Location now.", maxTokens: 2500 });
  let location;
  try {
...
async function createNewItem(worldId, { name, category, rarity, campaignContext } = {}) {
  ...
  const contentRaw = await callClaude({ systemPrompt: contentSystemPrompt, userMessage: "Generate the item now.", maxTokens: 2000 });
  let item;
  try {
...
async function createNewLog(worldId, { name, logType, campaignContext } = {}) {
  ...
  const contentRaw = await callClaude({ systemPrompt: contentSystemPrompt, userMessage: "Generate the log now.", maxTokens: 1500 });
  let log;
  try {
...
module.exports = { createNewNpc, createNewLocation, createNewItem, createNewLog };
