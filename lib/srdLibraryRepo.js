...
// Full-row read (data_json included) for small, whole-category use cases
// where every row's structured data is needed up front rather than just
// picker-list fields -- unlike listSrdEntries above, which deliberately
// selects only picker-safe columns. Species/Backgrounds are tiny
// categories (9/4 rows per scripts/ingestSrdOrigins5e.js), so selecting
// '*' for the whole category is cheap; not intended for large categories
// like spells/monsters -- use listSrdEntries + getSrdEntry for those.
async function listSrdEntriesFull(ruleset, category, { limit = 100 } = {}) {
  const { data, error } = await supabase
    .from("srd_library")
    .select("*")
    .eq("ruleset", ruleset)
    ...
    .order("name", { ascending: true })
    .limit(limit);
  if (error) throw new Error(`listSrdEntriesFull(${ruleset}/${category}) failed: ${error.message}`);
  return data || [];
}
...
module.exports = { listSrdEntries, listSrdEntriesFull, getSrdEntry, getSrdEntryBySlug, findNearestCrMonsters, recordImport, isAlreadyImported, listImportedEntryIds };
