...
async function generateGenericEnemyProcedurally(worldId, genericSystem, opts = {}) {
  ...
  const name = opts.name || `${pickGValue(enemiesTable.nameEpithets, buckets)} ${pickGValue(enemiesTable.nameNouns, buckets)}`;
  ...
  const enemy = {
    ...
    flavorStats: useFormula ? undefined : pickGValue(enemiesTable.flavorStatsTemplates, buckets),
    traits,
    actions,
...
async function generateGenericItemProcedurally(worldId, genericSystem, opts = {}) {
  const buckets = await detectGenreBuckets(worldId);
  const name = opts.name || pickGValue(itemsTable.names, buckets);
  ...
  const item = {
    ...
    description: pickGValue(itemsTable.descriptionTemplates, buckets),
...
async function generateGenericSurvivorProcedurally(worldId, genericSystem, opts = {}) {
  ...
  const pc = {
    ...
    attributes,
    derivedStats: useFormula ? computeDerivedStats(genericSystem, attributes) : null,
    flavorStats: useFormula ? undefined : pickGValue(survivorsTable.flavorStatsTemplates, buckets),
