...
const { getWorldClassNames, filterToWorldClasses } = require("../rulesets/5e/spellClasses");
...
async function generate5eSpellProcedurally(worldId, opts = {}) {
  ...
  // The seed table's classes are standard 5e names (Wizard, Cleric, ...),
  // which spawned ghost Class placeholders in worlds that don't have them
  // -- see lib/rulesets/5e/spellClasses.js. Keep exact matches; otherwise
  // attach one random existing class so the spell still belongs to
  // someone. No classes at all: none (no model here to invent a stub).
  const worldClassNames = await getWorldClassNames(worldId);
  let classes = filterToWorldClasses(row.classes, worldClassNames);
  if (!classes.length && worldClassNames.length) {
    classes = [worldClassNames[Math.floor(Math.random() * worldClassNames.length)]];
  }
