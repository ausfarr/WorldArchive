...
const { PROFICIENCY_BONUS_BY_LEVEL, ABILITY_SCORE_IMPROVEMENT_LEVELS, subclassUnlockLevel, spellSlotsForLevel, proficiencyBonusForLevel, matchCoreClassName, savingThrowProficienciesForClass, SKILLS } = require("../rulesets/5e/classFormulas");
...
const { CORE_BACKGROUNDS, CORE_FEATS } = require("../rulesets/5e/backgroundsAndFeats");
...
async function generate5eSurvivorProcedurally(worldId, opts = {}) {
  ...
  // R4 Phase 5: same hand-authored fallback lists AI Homebrew generation
  // now draws from -- a random background always, a random feat about
  // half the time once the rolled level is high enough to have reached
  // an ASI (the other half "took the ASI instead," same real-play
  // default the AI path uses for a null featKey).
  const background = CORE_BACKGROUNDS[Math.floor(Math.random() * CORE_BACKGROUNDS.length)];
  const eligibleForFeat = level >= FIRST_ASI_LEVEL;
  const feat = eligibleForFeat && Math.random() < 0.5 ? CORE_FEATS[Math.floor(Math.random() * CORE_FEATS.length)] : null;
