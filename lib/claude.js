...
// fails after all of parseJsonResponse's local repair attempts. A
// second consecutive parse failure almost always means something
// structural rather than bad luck, so this deliberately retries once,
// not in a loop -- a loop would just spend more tokens chasing the same
// outcome. On retry, maxTokens is bumped 50% and the message explicitly
// flags the previous failure, since a truncated response (hit the token
// limit mid-JSON) is the most common real cause reaching this point --
// parseJsonResponse's fallbacks handle malformed-but-complete JSON, not
// JSON missing its closing braces entirely. Every caller that generates
// structured content and expects JSON back should prefer this over the
// bare callClaude+parseJsonResponse pair, since a failure here means a
// generation-cap point and real token spend were already lost for
// nothing -- see session_addendum_campaign_structure_shipped.md's
// "not yet tested" note and the retry-once addendum for the context.
async function callClaudeExpectingJson({ systemPrompt, userMessage, maxTokens = 2000, model = CONTENT_MODEL }) {
  try {
