...
// Formats an assembleSessionContext() result's resolved entries as plain
// text roster context for a prompt -- shared by Session Packet (Phase 4)
// and Session Chronicle (Phase 5) generation, both of which need the
// same "here's what's actually in this Quest/Campaign" grounding block.
function formatRosterContextText(context) {
  const lines = [];
  ...
  if (!lines.length) return "(this Quest/Campaign has no entries referenced yet)";
  return lines.join("\n");
}

module.exports = { assembleSessionContext, getPriorChronicles, resolveQuestEntries, extractDungeonMaps, formatRosterContextText };
