...
function buildLocationContentSystemPrompt({ settingContext, loreContext, factionOptionsText, rosterContext, npcRosterText, name, regionBiome, faction, existingContent }) {
  ...
  return `You are generating a Location for a tabletop/game world archive — a real, specific place a player could travel to (a faction stronghold, a dungeon, a settlement, a ruin), not a vague region label. Output ONLY valid JSON matching the schema below — no markdown, no prose, no code fences.
...
FACTION VOICE: if this location belongs to a faction, let its territory visibly reflect that faction's established culture/philosophy from the world lore below — architecture, upkeep, signage, what's on display versus hidden. Don't default to a generic "evil empire base" or "noble rebel camp" look.
...
NOTABLE NPCS TIED HERE: reference existing NPCs from the roster below where a real connection exists — do not force-generate placeholders and do not invent an NPC id that isn't on the roster. It is fine and expected for this list to be empty for a brand-new location.
