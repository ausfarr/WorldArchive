...
IMPORTANT -- don't default to the literal English category name (or a close synonym of it) just because it's the safe choice. "Archery" and "Polearm" in particular read as medieval/fantasy no matter how they're spelled, so in a sci-fi, modern, or non-fantasy genre, actually reinvent the FORM rather than relabeling the same weapon with a fancier word:
- A pure-firearms/modern/sci-fi world has no real place for bows -- reinterpret "Archery" as something else entirely at the same power tier: thrown/guided munitions, deployable drones, a secondary ranged tech distinct from "Ballistics" (e.g. "Ordnance," "Payloads," "Deployables").
- Likewise "Polearm" in a non-fantasy world: reach/improvised melee tools, riot gear, industrial tools-as-weapons (e.g. "Reach Weapons," "Improvised Melee," "Rigging").
- If (and only if) the world's genre is genuinely medieval/fantasy/historical, it's fine for these to stay close to their literal form.
Sanity check before returning: for a modern or sci-fi genre, if either label still contains or closely echoes the words "archer," "bow," "polearm," "spear," or "lance," pick a different direction.

PAIRED/FAMILY NAMING (use this when a genre makes a category a genuine stretch on its own): "Polearm" and "Unarmed" are both melee at close-to-mid range, and "Archery" is a secondary ranged option alongside "Ballistics" -- you don't have to force each of the 7 into a fully independent-sounding word. When a category feels like a forced stretch by itself, it's fine to name it as part of a visible family with a related category instead, e.g. "Melee — Reach" (Polearm) paired with "Melee — Close Combat" (Unarmed), or "Archery" reframed as "Deployables & Munitions" sitting alongside "Ballistics" as "Rifles & Autocannons". This still needs exactly 7 distinct labels (one per category, each still balanced/clamped independently under its own canonical key) -- the pairing is a naming/flavor choice to make the family relationship read naturally, not a merge of the categories themselves.
